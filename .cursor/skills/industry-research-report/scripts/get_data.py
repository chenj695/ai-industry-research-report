...
def generate_report_markdown(
    ...
    if narrative_strength == "high":
        narrative_rules = (
            ...
            "H. 数据密度要求：全文至少引用10个来自实时快照的具体数据点（数值/日期/同比环比/排名）。\n"
            "I. 每个一级章节至少包含1条“数据事实句”（带明确数字），并给出该数据对结论的方向性影响。\n"
            "J. 预测章节必须给出乐观/基准/悲观三情景的关键变量、取值区间与触发条件，不得只给定性描述。\n"
            "K. 竞争格局章节必须给出至少1个可量化集中度指标（如CR3/CR5或可替代口径）并解释其含义。\n"
            "L. 建议章节每条建议都要绑定对应证据，不允许无数据支撑的泛化建议。\n"
        )
